...
public:
	softcatch_MotionPlanner(parabolaEstimator* PE, double period, const std::string& sysName = "softcatch_MotionPlanner") :
		...
		_vel_limit(2.25), _acc_limit_down(20),
...
//					_interPose.get<1>() = Eigen::Quaterniond(1,0,0,0);
				...
				_interVel = _objvel.get<0>() + _interCal.getInterTime()*_objacc;
				TargetHandNormVec = -1*(_interVel/_interVel.norm()) + 1*Eigen::Vector3d::UnitZ();
...
//			Ifclose(_handpose, _objpose, 0.15);
...
//				_IfBalancing = false;
		...
		case VEL_MATCHING:
			...
			if ( (_handVel.norm() >= _vel_limit || _objvel.get<0>().norm() >= (_vel_limit+0.5) || _Ifclose
					|| (-_handVel[2]+0.1 >= fabs(_objvel.get<0>()[2]) && Ifclose(_handpose, _objpose, 0.05) )
					 || !IfinSafeRegion() )
					&& _handVel[2] < 0){ // Conditions for exiting VEL_MATCHING

				if ( handstartmove ){
					_current_phase = ARRESTING;
					_prev_phase = VEL_MATCHING;
					std::cout<<"\nVEL_MATCHING-->ARRESTING!\n"
							<<"_vel_ref = "<<_vel<<", Ifclose = "<<_Ifclose<<"\n"
							<<"_handVel = "<<_handVel<<"\n"
							<<"_objvel = "<<_objvel.get<0>()<<"\n"
							<<"Time after apex is: "<<tf<<".\n";
					fflush(stdout);
				}
				else{
					if (_Ifclose){
						...
						_prev_phase = VEL_MATCHING;
						_IfBalancing = true;
						std::cout<<"\nCannot find a feasible acc, VEL_MATCHING-->STOPPING! And Balancing!\n"
								<<"_vel_ref = "<<_vel<<", Ifclose = "<<_Ifclose<<"\n"
								<<"_handVel = "<<_handVel<<"\n"
								<<"_objvel = "<<_objvel.get<0>()<<"\n"
								<<"Time after apex is: "<<tf<<".\n";
...
//			_acc[0] = pow((_objvel.get<0>()[0]-_vel[0]),2)*0.5/(_handpose.get<0>()[0] - _objpose.get<0>()[0] + 0.00/*0.07*/) + _objacc[0];
			...
			_acc[2] = pow((_objvel.get<0>()[2]-_handVel[2]),2)*0.5/(_handpose.get<0>()[2] - _objpose.get<0>()[2] + 0.03/*0.14*/) + _objacc[2];
			_acc[1] = pow((_objvel.get<0>()[1]-_handVel[1]),2)*0.5/(_handpose.get<0>()[1] - _objpose.get<0>()[1] + 0.00) + _objacc[1];
			_acc[0] = pow((_objvel.get<0>()[0]-_handVel[0]),2)*0.5/(_handpose.get<0>()[0] - _objpose.get<0>()[0] + 0.00/*0.07*/) + _objacc[0];
...
//				_acc[0] = _objvel.get<0>()[0]*_objvel.get<0>()[0]*0.5/(_handpose.get<0>()[0] - _objpose.get<0>()[0] + 0.035/*0.07*/) + _objacc[0];
				...
				if (/*_acc[2] < -12*/ tf >= tf_min/3 /*&& tf <= tf_max && tf >= tf_min*/){
...
//					}
			...
			static cp_type dLead;
			dLead = (_ret.get<0>() - _handpose.get<0>());
			if (dLead.norm() > 0.02){
				_ret = _retPrev;
			}
			else{
			...
			TargetHandNormVec = -1*(_objvel.get<0>()/_objvel.get<0>().norm()) + 1*Eigen::Vector3d::UnitZ();
		...
		case ARRESTING:
			...
			static bool IfCatchingPtTooLow = false;
...
//				init_pose = _retPrev;
				...
				_targetPose.get<0>() = init_pose.get<0>() + init_vel*(dis/init_vel.norm());
//				_targetPose.get<0>() = _handpose.get<0>() + init_vel*(dis/init_vel.norm());

				while(_targetPose.get<0>()[2] < 0.30){
//					dis = dis - 0.05;
					acc = acc + 0.5;
					_m2p.ChangeMaxAcc(acc);
					dis = pow(init_vel.norm(),2)/(2*_m2p.GetMaxAcc());
...
//			_targetPose.get<0>()[0] = _objpose.get<0>()[0];
//			_targetPose.get<0>()[1] = _objpose.get<0>()[1];
...
//			if (_m2p.Ifarrive()){
			if ( _m2p.Ifarrive() /*|| Ifclose(_handpose, _targetPose, 0.01)*/ || !IfinSafeRegion()
					|| _objpose.get<0>()[2] - _handpose.get<0>()[2] > -0.15 ) {
				_current_phase = STOPPING;
				_prev_phase = ARRESTING;

				std::cout<<"\nARRESTING-->STOPPING!\n m2p.ifarrive ="<<_m2p.Ifarrive()
						<<"\nInitial hand vel = "<<init_vel<<", dis = "<<dis << ", acc = "<<acc
						<<"\nTarget final point: "<<_targetPose.get<0>()
						<<"\nTime after apex is:"<<(getCurrentTime() - _t_apex)<<".\n";
				if(IfCatchingPtTooLow){
//					std::cout<<"\n****Catching point is too low! Fianl point is modified, probably won't catch!!!\n";
				}
//				fflush(stdout);
				_Ifm2pStart = false;
			}
...
//							<< "integral_err: "<< cp_err_integral
					...
					fflush(stdout);*/
					std::cout<<"\nIn STOPPING! And Balancing! [Enter] to throw back!";
					fflush(stdout);
				}
				else{
					std::cout<<"\nSTOPPING! \nReference tool pose: "<< _targetPose.get<0>()
