...
public:
	softcatch_MotionPlanner(parabolaEstimator* PE, double period, const std::string& sysName = "softcatch_MotionPlanner") :
		...
		_vel_limit(3.5), _acc_limit_down(-20),
...
//		_current_phase(STOPPING), _prev_phase(STOPPING),
	 ...
	 * */
	bool IfinSafeRegion(double z_min = 0.25, double z_max = 0.9,
...
//			_aaBuff = _handpose.get<1>();
//			_aaBuff = _aaBuff.inverse();
//			_handpose.get<1>() = _aaBuff;
		}
		else
...
//					_interPose.get<1>() = Eigen::Quaterniond(1,0,0,0);
				...
				_interVel = _objvel.get<0>() + _interCal.getInterTime()*_objacc;
				TargetHandNormVec = -1*(_interVel/_interVel.norm()) + Eigen::Vector3d::UnitZ();
...
//				_current_phase = STOPPING;
		...
		case VEL_MATCHING:
			...
			if (fabs(_vel[2]) >= _vel_limit || fabs(_vel[2]) >= fabs(_objvel.get<0>()[2]) || _Ifclose || !IfinSafeRegion()){
				if ( handstartmove ){
...
//				_Ifm2pStart = false;
			...
			_acc[1] = pow((_objvel.get<0>()[1]-_vel[1]),2)*0.5/(_handpose.get<0>()[1] - _objpose.get<0>()[1] + 0.03) + _objacc[1];
			_acc[0] = pow((_objvel.get<0>()[0]-_vel[0]),2)*0.5/(_handpose.get<0>()[0] - _objpose.get<0>()[0] + 0.03/*0.07*/) + _objacc[0];

			if (_acc[2] < _acc_limit_down)
				_acc[2] = _acc_limit_down;
...
//					}
			...
			TargetHandNormVec = -1*(_objvel.get<0>()/_objvel.get<0>().norm()) + Eigen::Vector3d::UnitZ();
...
//				init_vel = _vel;
//				init_vel[2] = _objacc[2]*_t_arr;

				_targetPose.get<0>() = _retPrev.get<0>() + init_vel*(0.15/init_vel.norm());
...
//			if (_m2p.Ifarrive()){
			if ( _m2p.Ifarrive() /*|| Ifclose(_handpose, _targetPose, 0.01)*/ || !IfinSafeRegion() ) {
				...
				printf("\nARRESTING-->STOPPING!\n"
						"m2p.ifarrive = %d; Ifclose = %d.\n"
						"Time after apex is: %4.3fs.\n",
						_m2p.Ifarrive(), _Ifclose, (getCurrentTime() - _t_apex));
				fflush(stdout);
//				_Ifm2pStart = false;
			}
			break;
...
//			_ret = _retPrev;
			...
			if (_objpose.get<0>()[2] - _ret.get<0>()[2] > -0.1 && IfBalancing == true){

				IfBalancing = true;

				cp_err = _targetPose.get<0>() - _objpose.get<0>();

				cp_err_integral = cp_err_integral + cp_err;
				...
				vel_err = Eigen::Vector3d::Zero() - _objvel.get<0>();

				angleX = -k_p*cp_err[1] - k_d*vel_err[1] - k_i*cp_err_integral[1];
				angleY = k_p*cp_err[0] + k_d*vel_err[0] + k_i*cp_err_integral[0];

				if (angleX > sat) angleX = sat; if (angleX < -sat) angleX = -sat;
				if (angleY > sat) angleY = sat; if (angleY < -sat) angleY = -sat;

				_targetPose.get<1>() = Eigen::Quaterniond(Eigen::AngleAxisd(angleX, Eigen::Vector3d::UnitX()))*
								Eigen::Quaterniond(Eigen::AngleAxisd(angleY, Eigen::Vector3d::UnitY()));
